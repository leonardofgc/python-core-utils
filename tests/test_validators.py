...
from core_utils.validators import (validate_not_empty, validate_positive_number, validate_email)
...
@pytest.mark.parametrize("email",["user@email.com","teste.silva@gmail.com","dev123@empresa.com"])
def test_validate_email_success(email):
    assert validate_email(email) == email


@pytest.mark.parametrize("email",["useremail.com","user@","@gmail.com","email-invalido"])
